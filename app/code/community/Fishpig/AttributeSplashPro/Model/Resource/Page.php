...
class Fishpig_AttributeSplashPro_Model_Resource_Page extends Mage_Core_Model_Resource_Db_Abstract
{
	...
	*/
			...
			foreach($optionFilters as $attribute => $data) {
				...
				if (isset($data['apply_to']) && $data['apply_to'] === 'simple') {
					if ($operator === 'OR' && count($values) > 1) {
						$this->_addEavAttributeOptionFilterToProductCollection($products, $attributeModel, $values, $page);
					}
					else {
	 ...
	 * @return $this
	 */
	protected function _addEavAttributeOptionFilterToProductCollection($products, $attribute, $value, $page)
	{
		...
		$products->getSelect()
			...
			->join(
				...
				. $this->_getReadAdapter()->quoteInto(" AND `{$alias}`.`value` " . (is_array($value) ? 'IN (?)' : ' = ?'), $value),
				''
			);
